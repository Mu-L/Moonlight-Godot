...
func _ready() -> void:
	...
	audio_left.unit_size = 20.0
	audio_left.max_distance = 50.0
	audio_left.attenuation_model = AudioStreamPlayer3D.ATTENUATION_INVERSE_DISTANCE
	
	audio_right.unit_size = 20.0
	audio_right.max_distance = 50.0
	audio_right.attenuation_model = AudioStreamPlayer3D.ATTENUATION_INVERSE_DISTANCE
	# -------------------
	...
	connect_button_signal("CheckButton2", "toggled", func(t): godot_audio_enabled = t) # Godot Audio Switch
...
func test_establish_stream() -> void:
	...
	if audio_enabled:
		...
		if godot_audio_enabled:
			...
			if streams.size() > 0:
				audio_left.stream = streams[0]
				audio_left.play()
				print("Assigned Left Audio Stream")
			if streams.size() > 1:
				audio_right.stream = streams[1]
				audio_right.play()
				print("Assigned Right Audio Stream")
		else:
			print("Godot Audio Disabled (Maybe using Bypass or just Muted)")
