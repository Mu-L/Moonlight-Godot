...
int MoonlightStreamCore::_handle_ar_init(int audio_cfg, const POPUS_MULTISTREAM_CONFIGURATION opus_config) {
	...
	if (opus_config && opus_config->channelCount > 2) {
		...
		// FFmpeg treats the mapping array in OpusHead as mapping to Vorbis channel order.
		// Since opus_config->mapping maps to Limelight (SMPTE) order, we must translate it to Vorbis order.
		uint8_t vorbis_mapping[AUDIO_CONFIGURATION_MAX_CHANNEL_COUNT];
		memset(vorbis_mapping, 0, sizeof(vorbis_mapping));
		if (opus_config->channelCount == 6) {
			vorbis_mapping[0] = opus_config->mapping[0]; // FL
			vorbis_mapping[1] = opus_config->mapping[2]; // C
			vorbis_mapping[2] = opus_config->mapping[1]; // FR
			vorbis_mapping[3] = opus_config->mapping[4]; // SL
			vorbis_mapping[4] = opus_config->mapping[5]; // SR
			vorbis_mapping[5] = opus_config->mapping[3]; // LFE
		} else if (opus_config->channelCount == 8) {
			vorbis_mapping[0] = opus_config->mapping[0]; // FL
			vorbis_mapping[1] = opus_config->mapping[2]; // C
			vorbis_mapping[2] = opus_config->mapping[1]; // FR
			vorbis_mapping[3] = opus_config->mapping[6]; // SL
			vorbis_mapping[4] = opus_config->mapping[7]; // SR
			vorbis_mapping[5] = opus_config->mapping[4]; // RL
			vorbis_mapping[6] = opus_config->mapping[5]; // RR
			vorbis_mapping[7] = opus_config->mapping[3]; // LFE
		} else {
			memcpy(vorbis_mapping, opus_config->mapping, opus_config->channelCount);
		}

		// Channel Mapping Table (N bytes)
		memcpy(data + 21, vorbis_mapping, opus_config->channelCount);
						...
						for (int ch = 0; ch < expected_ch; ch++) {
							if (ch < (int)audio_streams.size() && audio_streams[ch].is_valid()) {
								...
								for (int s = 0; s < out_samples_multi; s++) {
									chan_buf[s] = multi_buf[s * expected_ch + ch];
								}
